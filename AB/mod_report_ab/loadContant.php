...
<body>
    ...
    <form action="?" method="post" name="myForm" id="myForm">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top:20px;" align="center">
                ...
                <tr>
                    ...
                    <select name="inputSellID" id="inputSellID" onchange="document.myForm.submit();" class="formSelectSearchStyle">
                        ...
                        foreach ($arrSellers as $seller) {
                            $selected = ($_REQUEST['inputSellID'] == $seller['sellid_raw']) ? 'selected="selected"' : '';
                            echo '<option value="' . $seller['sellid_raw'] . '" ' . $selected . '>' . $seller['subject'] . '</option>';
                        }
                        ?>
                    </select>
        ...
        <div class="divRightMain flexd"> <!--ส่วนเนื้อหารายงานหลัก-->
                ...
                <table width="15%" border="0" cellspacing="0" cellpadding="0" align="left" class="tbBoxListwBorder">
                    ...
                    while ($day < $countDay + 1) {
                        ...
                        if ($day <= $countDayNow +1) {  
                            $apply = countRegisterBySellID($year, $month, $day, $Sellid); //นับยอดรวมการสมัครฝากของ sellid ในทีม
                            $deposit = countDepositBySellID($year, $month, $day, $Sellid);     
                        }else{
                            $apply = 0;
                            $deposit = 0;
                        }                       
            ...
            <?php
            // ดึง sellid ของทีมตาม masterkey ที่กำหนด
            $arrSellers = getSellersByTeam($masterkey);
            ?>
                ...
                <table class="table-main" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                        ...
                        <?php
                            ...
                            // ถ้ามีการเลือก sellid ให้ filter array เหลือแค่ sellid ที่เลือก
                            if (!empty($_REQUEST['inputSellID']) && $_REQUEST['inputSellID'] > 0) {
                                $arrSellers = array_filter($arrSellers, function($seller) {
                                    return $seller['sellid_raw'] == $_REQUEST['inputSellID'];
                                });
                            }
                        ?>
                            
                            <!--ส่วนของการแสดงตารางสมัคร-ฝาก-->
                            <?php foreach ($arrSellers as $seller) { ?>
                        ...
                        while ($day <= $countDay) { // ใช้ <= เพื่อรวมวันสุดท้าย
                            ...
                            $dayPad = sprintf('%02d', $day); // แปลงวันเป็น 2 หลัก
                            $monthPad = sprintf('%02d', $month); // แปลงเดือนเป็น 2 หลัก
                        ?>
                            <tr class="<?php echo $valDivTr ?> table-list">
                                <?php foreach ($arrSellers as $seller) { ?>
                                    <td class="divRightContantOverTb">
                                    <span class="reg-<?php echo $seller['id'] ?>-date-<?php echo $year ?>-<?php echo $monthPad ?>-<?php echo $dayPad ?>">0</span>
                                    </td>
                                    <td class="divRightContantOverTb">
                                        <span class="dep-<?php echo $seller['id'] ?>-date-<?php echo $year ?>-<?php echo $monthPad ?>-<?php echo $dayPad ?>">0</span>
                                    </td>
                                <?php } ?>
                            </tr>
                        <?php $day++; } ?>
