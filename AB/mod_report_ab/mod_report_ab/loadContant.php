...
$valNav2 = getNameMenu($_REQUEST["menukeyid"]); // ชื่อเมนูหลักของรายงาน AB


$valPermission = getUserPermissionOnMenu($_SESSION[$valSiteManage . "core_session_groupid"], $_REQUEST["menukeyid"]);

// masterkey ของ module นี้ (อาจจะไม่ใช่ HA โดยตรง แต่ใช้ในการ query อื่นๆ)
$masterkey = $_REQUEST['masterkey'];

/*$masterkey = $_REQUEST['masterkey'];
if ($masterkey == "HA") { // masterkey สำหรับทีม HA
    $arrayTeamData = $arrayTeamAB ['HA']; 
} elseif($masterkey == "HB") { // masterkey สำหรับทีม HB
    $arrayTeamData = $arrayTeamAB ['HB']; 
}*/
...
<body>
    ...
    <form action="?" method="post" name="myForm" id="myForm">
                ...
                <tr>
                    ...
                    <td class="selectSearch2">
                        <select name="inputSellID" id="inputSellID" onchange="document.myForm.submit();" class="formSelectSearchStyle">
                            ...
                            if ($valUserGroupid == 62) {
                                $sql_sell = $sql_sell . " AND ((" . $mod_tb_root . "_sellid ='" . $valUserID . "')  ";
                            }elseif ($valUserGroupid == 70) {
                                $sql_sell = $sql_sell . " AND (" . $mod_tb_root . "_sellid IN (" . $valArrSell . ") )  ";
                            }
                            ...
                            if ($valUserGroupid == 62) {
                                $sql_sell_search = $sql_sell_search . " AND ((" . $mod_tb_root . "_sellid ='" . $valUserID . "')  ";
                            }elseif ($valUserGroupid == 70) {
                                $sql_sell_search = $sql_sell_search . " AND (" . $mod_tb_root . "_sellid IN (" . $valArrSell . ") )  ";
                            }
        ...
        <div class="divRightMain flexd"> <!--ส่วนเนื้อหารายงานหลัก-->
                ...
                <table width="15%" border="0" cellspacing="0" cellpadding="0" align="left" class="tbBoxListwBorder">
                    ...
                    while ($day < $countDay + 1) {
                        ...
                        if ($day <= $countDayNow +1) {  
                            $apply = countRegisterBySellID($year, $month, $day, $dvid, $valArrSell); //นับยอดรวมการสมัครฝากของ sellid ในทีม
                            $deposit = countDepositBySellID($year, $month, $day, $dvid, $valArrSell);     
                        }else{
                            $apply = 0;
                            $deposit = 0;
                        }                       
            ...
            <?php
            // ดึง sellid ทีม A
            $arrSellersTeamA = getSellersTeamA();
            ?>

            <div class="table-scoll"> <!--ตารางด้านขวา-->
                <table class="table-main" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                        <tr class="table-header"> <!--ชื่อตาม sellid-->
                            <?php foreach ($arrSellersTeamA as $seller) { ?>
                                <th width="20%" colspan="2" class="divRightTitleTb"><?php echo $seller['subject']; ?></th>
                            <?php } ?>
                        </tr>
                        
                        <tr class="table-sub-header"> <!--ตารางสมัคร ฝาก-->
                            <?php foreach ($arrSellersTeamA as $seller) { ?>
                                <td width="12%" class="divRightTitleTb left" valign="middle" align="center">
                                    <span class="fontTitlTbRight"><?php echo $langMod["tit:regis"] ?></span>
                                </td>
                                <td width="12%" class="divRightTitleTb right" valign="middle" align="center">
                                    <span class="fontTitlTbRight"><?php echo $langMod["tit:deposit"] ?></span>
                                </td>
                            <?php } ?>
                        </tr>
                        ...
                        $valDivTr = ($valDivTr == "divSubOverTb") ? "divOverTb" : "divSubOverTb";
                        ?>

                        <tr class="<?php echo $valDivTr ?> table-list">    <!--แสดงผลยอดสมัครฝากในตาราง-->              
                        <?php foreach ($arrSellersTeamA as $seller) { ?> 
                            <td class="divRightContantOverTb">
                                <span class="reg-<?php echo $seller['id'] ?>-date-<?php echo $year ?>-<?php echo $month ?>-<?php echo $day ?>">0</span>
                            </td>
                            <td class="divRightContantOverTb">
                                <span class="dep-<?php echo $seller['id'] ?>-date-<?php echo $year ?>-<?php echo $month ?>-<?php echo $day ?>">0</span>
                            </td>
                        <?php } ?>
                        </tr>
                        ...
                        <tr class="table-sum"> <!--Summary Row: ตารางสรุปยอดรวมของเดือน-->  
                        <?php foreach ($arrSellersTeamA as $seller) { ?>
                            <td class="divRightContantOverTb" valign="top" align="center">
                                <span class="fontContantTbTime sumReg-<?php echo $seller['id'] ?>">0</span>
                            </td>
                            <td class="divRightContantOverTb" valign="top" align="center">
                                <span class="fontContantTbTime sumDep-<?php echo $seller['id'] ?>">0</span>
                            </td>
                        <?php } ?>
                        </tr>
                        ...
                        <?php foreach ($arrSellersTeamA as $seller) { ?>
                            <th colspan="2" class="divRightTitleTb"><?php echo $seller['subject']; ?></th>
                        <?php } ?>
                        </tr>
                    </tbody>
                </table>
