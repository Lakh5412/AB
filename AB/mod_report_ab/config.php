...
// ฟังก์ชันสำหรับดึงรายชื่อ Seller ของทีม A หรือทีม B ตาม Masterkey ที่กำหนด
function getSellersByTeam($masterkey)
{
    ...
    $sql .= " AND ( team." . $mod_tb_data . "_subject LIKE '" . $masterkey . "%' ) "; // กรองทีมที่ขึ้นต้นด้วย masterkey
...
function countRegisterBySellID($year = null, $month = null, $day = null, $sellid = 0, $masterkey = '') // เพิ่ม $masterkey
{
    global $coreLanguageSQL, $mod_tb_root;

    if ($sellid <= 0) return 0;
    ...
    // เพิ่มเงื่อนไขกรอง masterkey
    if (!empty($masterkey)) {
        $sql .= " AND " . $mod_tb_root . "_masterkey = '" . wewebEscapeSQL($coreLanguageSQL, $masterkey) . "'";
    }
    // อาจจะต้องเพิ่มเงื่อนไขสถานะการสมัครที่ถูกต้องด้วย หากจำเป็น
    // $sql .= " AND " . $mod_tb_root . "_statusid IN (1, 188)"; 
...
function countDepositBySellID($year = null, $month = null, $day = null, $sellid = 0, $masterkey = '') // เพิ่ม $masterkey
{
    global $coreLanguageSQL, $mod_tb_root;
    ...
    $sql .= " AND " . $mod_tb_root . "_sellid = '" . $sellid . "'";
    $sql .= " AND " . $mod_tb_root . "_statusid IN (187,188)"; // สถานะฝากสำเร็จ
    // เพิ่มเงื่อนไขกรอง masterkey
    if (!empty($masterkey)) {
        $sql .= " AND " . $mod_tb_root . "_masterkey = '" . wewebEscapeSQL($coreLanguageSQL, $masterkey) . "'";
    }
